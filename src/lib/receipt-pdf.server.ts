...
export interface ReceiptPdfData {
  ...
  /** Privat andel i procent (0-100) på et erhvervsbilag. Null = ingen deling. */
...
const L = {
  da: {
    ...
    privateShare: "Privat andel",
    businessShare: "Erhvervsandel",
    businessShareVat: "Erhvervsandel moms",
  ...
  en: {
    ...
    businessShareVat: "Business share VAT",
...
export async function generateReceiptPdf(data: ReceiptPdfData): Promise<Uint8Array> {
  ...
  // ---------- Privat andel / erhvervsandel ----------
  const sharePct =
    data.private_share_pct == null || !Number.isFinite(Number(data.private_share_pct))
      ? null
      : Math.min(100, Math.max(0, Number(data.private_share_pct)));
  let shareBottom = ty - 12;
  if (sharePct != null && sharePct > 0) {
    const bizFactor = (100 - sharePct) / 100;
    const bizAmount = Math.round(gross * bizFactor * 100) / 100;
    const bizVat = hasVat ? Math.round(vat * bizFactor * 100) / 100 : null;
    let sy = ty - 26;
    const rows: Array<[string, string]> = [
      [t.privateShare, `${String(sharePct).replace(".", ",")}%`],
      [t.businessShare, fmtMoney(bizAmount, currency, lang)],
    ];
    if (bizVat != null) rows.push([t.businessShareVat, fmtMoney(bizVat, currency, lang)]);
    for (const [k, v] of rows) {
      drawRight(k, boxX + 150, sy, BODY, font, MUTED);
      drawRight(v, right, sy, BODY, font, INK);
      sy -= 14;
    }
    shareBottom = sy;
  }

  y = Math.min(boxY, shareBottom - 24);
